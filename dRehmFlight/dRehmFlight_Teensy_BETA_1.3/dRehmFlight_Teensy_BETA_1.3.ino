...
unsigned long channel_5_fs = 1000; //ch5
unsigned long channel_6_fs = 1000; //ch6
...
float GyroErrorX = -3.01;
float GyroErrorY = 0.38;
float GyroErrorZ = -0.13;

//Controller parameters (take note of defaults before modifying!): 
float i_limit = 25.0;     //Integrator saturation level, mostly for safety (default 25.0)
float maxRoll = 180.0;     //Max roll angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode 
float maxPitch = 180.0;    //Max pitch angle in degrees for angle mode (maximum ~70 degrees), deg/sec for rate mode
...
//For the SU-30 rate-controller, these are working values but most likely not optimal
float Kp_roll_rate = 0.23;    //Roll P-gain - rate mode
float Ki_roll_rate = 0.0;     //Roll I-gain - rate mode
float Kd_roll_rate = 0.0002;  //Roll D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)
float Kp_pitch_rate = 0.23;   //Pitch P-gain - rate mode
float Ki_pitch_rate = 0.0;    //Pitch I-gain - rate mode
float Kd_pitch_rate = 0.0002; //Pitch D-gain - rate mode (be careful when increasing too high, motors will begin to overheat!)

float Kp_yaw = 0.45;           //Yaw P-gain
...
void controlMixer() {
  ...
  //Left and right according to back view
  //Negating both roll values  on a servo pair reverses roll direction, negating both pitch values reverses pitch direction.

  if (channel_6_pwm < 1500 && channel_5_pwm < 1500) { //Normal unstabilized
    s1_command_scaled = thro_des; //Throttle Passthrough
    s2_command_scaled = 0.5 - roll_passthru + pitch_passthru; //Stabilator Right
    s3_command_scaled = 0.5 - roll_passthru - pitch_passthru; //Stabilator Left
    s4_command_scaled = 0.5; //TV Vertical Right disabled
    s5_command_scaled = 0.5; //TV Vertical Left disabled
    s6_command_scaled = 0.5; //TV Horizontal disabled
    s7_command_scaled = 0;
  }
  else if (channel_6_pwm < 1500 && channel_5_pwm > 1500) { //Normal stabilized
    s1_command_scaled = thro_des; //Throttle Passthrough
    s2_command_scaled = 0.5 - (roll_passthru + roll_PID) + (pitch_passthru + pitch_PID); //Stabilator Right Stabilized
    s3_command_scaled = 0.5 - (roll_passthru + roll_PID) - (pitch_passthru + pitch_PID); //Stabilator Left Stabilized
    s4_command_scaled = 0.5; //TV Vertical Right disabled
    s5_command_scaled = 0.5; //TV Vertical Left disabled
    s6_command_scaled = 0.5; //TV Horizontal disabled
    s7_command_scaled = 0;
  }  
  else if (channel_6_pwm > 1500 && channel_5_pwm < 1500) { //Thrust vectoring unstabilized
    s1_command_scaled = thro_des; //Throttle Passthrough
    s2_command_scaled = 0.5 - roll_passthru + pitch_passthru; //Stabilator Right
    s3_command_scaled = 0.5 - roll_passthru - pitch_passthru; //Stabilator Left
    s4_command_scaled = 0.5 + roll_passthru - pitch_passthru; //TV Vertical Right
    s5_command_scaled = 0.5 + roll_passthru + pitch_passthru; //TV Vertical Left
    s6_command_scaled = 0.5 + yaw_passthru; //TV Horizontal Both
    s7_command_scaled = 0;    
  }
  else if (channel_6_pwm > 1500 && channel_5_pwm > 1500) { //Thrust vectoring stabilized
    s1_command_scaled = thro_des; //Throttle Passthrough
    s2_command_scaled = 0.5 - (roll_passthru + roll_PID) + (pitch_passthru + pitch_PID); //Stabilator Right Stabilized
    s3_command_scaled = 0.5 - (roll_passthru + roll_PID) - (pitch_passthru + pitch_PID); //Stabilator Left Stabilized
    s4_command_scaled = 0.5 + (roll_passthru + roll_PID) - (pitch_passthru + pitch_PID); //TV Vertical Right Stabilized
    s5_command_scaled = 0.5 + (roll_passthru + roll_PID) + (pitch_passthru + pitch_PID); //TV Vertical Left Stabilized
    s6_command_scaled = 0.5 + yaw_passthru + yaw_PID; //TV Horizontal Both
    s7_command_scaled = 0;
  }  
}
